...
Raw paired-end FASTQ files
        ↓
Trim Galore (adapter trimming + quality control)
        ↓
Bismark (genome preparation + alignment)
        ↓
Deduplication & BAM filtering
        ↓
Methylation extraction (CpG / CHG / CHH)
        ↓
BedGraph generation
        ↓
BigWig conversion for visualization
        ↓
Genome-wide methylation profiling (ViewBS)
        ↓
Gene-level methylation analysis (bedtools + R)
